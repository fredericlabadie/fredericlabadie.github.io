# dbt + BigQuery Portfolio Project Plan
...
- **Warehouse:** BigQuery (GCP — existing account, no setup friction)
- **Transformation:** dbt Cloud (free developer tier — cloud.getdbt.com)
- **Source control:** GitHub — new public repo `hospital-readmissions-dbt`
- **Visualisation:** Looker Studio connected via BigQuery connector

---

## BigQuery Setup

1. Create a new BigQuery dataset: `hospital_analytics`
2. Create schemas (datasets): `raw`, `staging`, `intermediate`, `marts`
3. Load CMS CSV into `raw.hospital_readmissions` via BigQuery UI or `bq load`
4. Connect dbt Cloud to BigQuery via service account JSON credentials
5. Set `project` and `dataset` in dbt `profiles.yml`
...
In a Philips device context: which clinical sites are adopting new device features or workflow changes, and which are not? The dbt architecture here surfaces that signal from outcome data — the same pattern applies to telemetry and usage data from medical devices.
...
| Task | Time |
|------|------|
| BigQuery setup + data load | 30 minutes |
...
Recommended split: two sessions. Session 1: BigQuery setup + staging layer. Session 2: intermediate + marts + documentation.

---

## Build Tool

Use **Claude Code** for all model writing and debugging. Open with the ForAI.md prompt in this folder.
...
```
# Hospital Readmissions Analytics — dbt + BigQuery
...
The "Real-World Extension" section notes that this architecture is compatible with MIMIC-III (PhysioNet) — the gold standard clinical dataset — and explains what additional models that would enable. Signals clinical domain awareness without requiring credentialed access.
...
- **Analytics engineering** — dbt three-layer architecture on BigQuery, production-equivalent tooling
- **Adoption metrics thinking** — clinical workflow adoption maps directly to "Adoption & Insights" in the Philips role title
...
*Plan version: May 2026 — updated stack to BigQuery (native GCP environment, no setup friction)*
